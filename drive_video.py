...
class _BrightnessAutoController:
    """Target-brightness controller: startup baseline + online gain trim."""
    ...
    def __init__(self, device_path: str, target: float = 110.0, tolerance: float = 15.0):
        self.device_path = str(device_path)
        self.target = float(target)
        self.tolerance = float(tolerance)
        self.last_mean = 0.0
        self.last_stats = {"mean": 0.0, "median": 0.0, "p10": 0.0, "p90": 0.0}
        self._vendor_mode = 2
        self._last_online_adjust_ts = 0.0
        self._last_rebaseline_ts = 0.0
        self._out_of_band_cnt = 0

    @staticmethod
    def _ioc(dir_: int, type_char: str, nr: int, size: int) -> int:
        ioc_nrbits = 8
        ioc_typebits = 8
        ioc_sizebits = 14
        ioc_nrshift = 0
        ...
        return (dir_ << ioc_dirshift) | (ord(type_char) << ioc_typeshift) | (nr << ioc_nrshift) | (size << ioc_sizeshift)

    @classmethod
    def _iowr(cls, type_char: str, nr: int, size: int) -> int:
        return cls._ioc((1 | 2), type_char, nr, size)

    def _xu_query(self, unit: int, selector: int, query: int, payload):
        ioctl_cmd = self._iowr("u", 0x21, ctypes.sizeof(self._UvcXuControlQuery))
        buf = (ctypes.c_uint8 * len(payload))(*payload)
        q = self._UvcXuControlQuery(
            unit=unit,
            selector=selector,
            query=query,
            size=len(payload),
    ...
    def _set_vendor_mode(self, mode: int) -> bool:
        m = int(max(0, min(3, int(mode))))
        ok = self._xu_query(unit=3, selector=1, query=0x01, payload=[m])
        if ok:
            self._vendor_mode = m
        return ok

    def _v4l2_set(self, name: str, value: int) -> bool:
        try:
            subprocess.run(
                ["v4l2-ctl", "-d", self.device_path, "-c", f"{name}={int(value)}"],
                check=True,
                capture_output=True,
                text=True,
            )
    ...
    def _v4l2_get(self, name: str):
        try:
            result = subprocess.run(
                ["v4l2-ctl", "-d", self.device_path, "-C", name],
                check=True,
                capture_output=True,
                text=True,
            )
    ...
    def _set_control(self, name: str, value: int) -> bool:
        if name == "vendor_exposure_mode":
            return self._set_vendor_mode(int(value))
        return self._v4l2_set(name, int(value))

    def _get_control(self, name: str):
        if name == "vendor_exposure_mode":
            return float(self._vendor_mode)
        return self._v4l2_get(name)

    @classmethod
    def _roi_stats(cls, frame_bgr):
        gray = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2GRAY)
        h, w = gray.shape[:2]
        pts = np.array([[int(px * w), int(py * h)] for px, py in cls._TRAPEZOID_NORM], dtype=np.int32)
    ...
    def _measure(self, cap, settle_frames: int = 2):
        frame = None
        ...
        if frame is None:
            return None, None
        stats = self._roi_stats(frame)
        mean = float(stats["mean"])
        self.last_mean = mean
        self.last_stats = stats
        return mean, stats

    def run_startup_baseline(self, cap, max_steps: int = 24):
        # Keep camera geometry unchanged; only brightness-related controls.
        self._v4l2_set("auto_exposure", 2)
        param_order = [
            ("vendor_exposure_mode", 1, 0, 3),
            ("brightness", 8, 0, 100),
            ("gamma", 1, 0, 4),
            ("contrast", 5, 0, 100),
            ("backlight_compensation", 1, 0, 1),
        ]
        curr = {}
        for name, _, mn, mx in param_order:
            v = self._get_control(name)
            if v is None:
                continue
            iv = int(round(v))
            curr[name] = int(max(mn, min(mx, iv)))
        if "gain" not in curr:
            g = self._get_control("gain")
            curr["gain"] = int(round(g if g is not None else 6))

        mean, _ = self._measure(cap, settle_frames=3)
        if mean is None:
            return False

        steps = 0
        for _ in range(max_steps):
            err = self.target - float(mean)
            if abs(err) <= self.tolerance:
                break
            direction = 1 if err > 0 else -1
            improved = False
            for name, step, mn, mx in param_order:
                if name not in curr:
                    continue
                old = int(curr[name])
                candidates = [int(max(mn, min(mx, old + direction * step))), int(max(mn, min(mx, old - direction * step)))]
                candidates = [c for c in candidates if c != old]
                best_mean = float(mean)
                best_val = old
                best_err = abs(self.target - best_mean)
                for cand in candidates:
                    steps += 1
                    if not self._set_control(name, cand):
                        continue
                    m2, _ = self._measure(cap, settle_frames=2)
                    if m2 is None:
                        continue
                    e2 = abs(self.target - float(m2))
                    if e2 + 1e-6 < best_err:
                        best_err = e2
                        best_val = cand
                        best_mean = float(m2)
                if best_val != old:
                    ...
                    improved = True
                    break
                self._set_control(name, old)
            if not improved:
                break
            if steps >= (max_steps * 3):
                break

        # Fine stage: gain only.
        gain = int(round(self._get_control("gain") or curr.get("gain", 6)))
        for _ in range(12):
            mean, _ = self._measure(cap, settle_frames=1)
            if mean is None:
                break
            err = self.target - float(mean)
            if abs(err) <= self.tolerance:
                break
            gain = int(max(0, min(15, gain + (1 if err > 0 else -1))))
            self._set_control("gain", gain)
        self._last_rebaseline_ts = time.monotonic()
        return True

    def online_tick(self, cap, frame_bgr):
        now = time.monotonic()
        if frame_bgr is None:
            return
        stats = self._roi_stats(frame_bgr)
        mean = float(stats["mean"])
        self.last_mean = mean
        self.last_stats = stats
        err = self.target - mean
        if abs(err) <= self.tolerance:
            self._out_of_band_cnt = 0
            return
        self._out_of_band_cnt += 1
        if (now - self._last_online_adjust_ts) >= 0.5 and self._out_of_band_cnt >= 2:
            g = self._get_control("gain")
            if g is not None:
                gain = int(round(float(g)))
                gain_new = int(max(0, min(15, gain + (1 if err > 0 else -1))))
                if gain_new != gain:
                    self._set_control("gain", gain_new)
                    self._last_online_adjust_ts = now
                    self._out_of_band_cnt = 0
                    return
        # if gain saturated and still bad for long time -> run short baseline retune
        if (now - self._last_rebaseline_ts) > 20.0 and self._out_of_band_cnt >= 8:
            self.run_startup_baseline(cap, max_steps=12)
            self._out_of_band_cnt = 0
...
class VideoController(QtCore.QObject):
    ...
    def __init__(
        ...
        self._bright_ctl: Optional[_BrightnessAutoController] = None
        self._bright_global_done = False
        self._bright_tick_div = 0
        # fail-safe: stop if visual guidance is lost for too long
        self._nav_loss_t0 = None
        self._nav_loss_kind = None
        # fps
    ...
    def set_nav_mode(self, nav_mode: str):
        ...
        self._clear_nav_loss_watchdog()
        self._ensure_camera_for_current_mode()
        # seg is needed only for road mode (but we keep lazy-load)
        print(f"[DRIVE] nav_mode -> {nav_mode}", flush=True)

    def _clear_nav_loss_watchdog(self):
        self._nav_loss_t0 = None
        self._nav_loss_kind = None

    def _handle_nav_loss_watchdog(self, *, lost: bool, kind: str, timeout_s: float = 10.0):
        st = self.state
        if (not bool(getattr(st, "is_running", False))) or bool(getattr(st, "manual_drive", False)):
            self._clear_nav_loss_watchdog()
            return
        ...
        now = time.monotonic()
        if not lost:
            if self._nav_loss_t0 is not None:
                dt = now - float(self._nav_loss_t0)
                print(f"[SAFE] recovered {kind} after {dt:.1f}s", flush=True)
            self._clear_nav_loss_watchdog()
            return

        if (self._nav_loss_t0 is None) or (self._nav_loss_kind != kind):
            self._nav_loss_t0 = now
            self._nav_loss_kind = kind
            print(f"[SAFE] lost {kind} -> start {timeout_s:.0f}s timer", flush=True)
            return
        ...
        # hard stop once timeout reached
        st.is_running = False
        st.safety_stop = True
        try:
            ...
            print("[SAFE] motors_set stop error:", e, flush=True)
        print(f"[SAFE] STOP: {kind} lost for {dt:.1f}s (timeout={timeout_s:.0f}s)", flush=True)
        self._clear_nav_loss_watchdog()
    ...
    def _open_camera(self, dev, w: int, h: int, fps: int) -> bool:
        ...
        try:
            ...
            try:
                dev_path = open_dev if isinstance(open_dev, str) else f"/dev/video{int(open_dev)}"
                if self._bright_ctl is None or self._bright_ctl.device_path != dev_path:
                    self._bright_ctl = _BrightnessAutoController(
                        device_path=dev_path,
                        target=float(getattr(self.state, "cam_target_brightness", 110.0) or 110.0),
                        tolerance=float(getattr(self.state, "cam_target_tolerance", 15.0) or 15.0),
                    )
                self._bright_ctl.target = float(getattr(self.state, "cam_target_brightness", 110.0) or 110.0)
                self._bright_ctl.tolerance = float(getattr(self.state, "cam_target_tolerance", 15.0) or 15.0)
                if not self._bright_global_done:
                    ok_tune = self._bright_ctl.run_startup_baseline(cap, max_steps=24)
                    self._bright_global_done = bool(ok_tune)
                    print(
                        f"[CAM][AUTO-BRIGHT] startup global tune done={ok_tune} "
                        f"target={self._bright_ctl.target:.1f} tol=+/-{self._bright_ctl.tolerance:.1f} "
                        f"mean={self._bright_ctl.last_mean:.1f}",
                        flush=True,
                    )
            except Exception as e:
                print("[CAM][AUTO-BRIGHT] init error:", e, flush=True)
    ...
    def _update_telem_labels(self):
        ...
        roi_mean = getattr(st, "cam_roi_mean", None)
        try:
            roi_txt = f"{float(roi_mean):.1f}" if roi_mean is not None else "—"
        except Exception:
            roi_txt = "—"
        self._lblTelemRow2.setText(f"STEER {steer:+.2f}   ROI {roi_txt}")
    ...
    def _camera_tick(self):
        ...
        if not ok or frame is None:
            ...
            if self._cam_fail_cnt >= 10:
                ...
                self._cam_fail_cnt = 0
            return
        # Keep pipeline orientation consistent with camera_debug and field mount.
        ...
        st = self.state
        nav_mode = self._current_nav_mode()
        if (not bool(getattr(st, "is_running", False))) or bool(getattr(st, "manual_drive", False)) or bool(getattr(st, "trajectory_mode", False)):
            self._clear_nav_loss_watchdog()
        # Online minimal correction during driving: gain-only micro-adjust + rare short rebaseline.
        try:
            if bool(getattr(st, "is_running", False)) and self._bright_ctl is not None:
                self._bright_ctl.target = float(getattr(st, "cam_target_brightness", 110.0) or 110.0)
                self._bright_ctl.tolerance = float(getattr(st, "cam_target_tolerance", 15.0) or 15.0)
                self._bright_tick_div = (int(self._bright_tick_div) + 1) % 3
                if self._bright_tick_div == 0:
                    self._bright_ctl.online_tick(self._cam, frame)
                    st.cam_roi_mean = float(self._bright_ctl.last_stats.get("mean", 0.0))
                    st.cam_roi_median = float(self._bright_ctl.last_stats.get("median", 0.0))
        except Exception:
            pass
        ...
        if nav_mode == MODE_MARKERS:
            ...
            if is_run and (not is_manual) and (not is_traj):
                ...
                marker_lost = not bool(getattr(st, "marker_seen", False))
                self._handle_nav_loss_watchdog(lost=marker_lost, kind="marker/sign", timeout_s=10.0)
            else:
                self._clear_nav_loss_watchdog()
        ...
        if nav_mode == MODE_ROAD:
            ...
            if not use_seg:
                ...
                if bool(getattr(st, "is_running", False)) and (not bool(getattr(st, "manual_drive", False))):
                    self._handle_nav_loss_watchdog(lost=True, kind="road", timeout_s=10.0)
                else:
                    self._clear_nav_loss_watchdog()
                return self._render_camera(frame, mask=None, road_bin=None, road_support=0.0, thr=0.0)
            ...
            if bool(getattr(st, "is_running", False)) and (not bool(getattr(st, "manual_drive", False))):
                ...
                road_lost = not bool(getattr(st, "road_detected", False))
                self._handle_nav_loss_watchdog(lost=road_lost, kind="road", timeout_s=10.0)
            else:
                self._clear_nav_loss_watchdog()
